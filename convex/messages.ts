...
const populationThread =async(ctx:QueryCtx,messageId:Id<"messages">)=>{
...
const populationReaction =async(ctx:QueryCtx,messageId:Id<"messages">)=>{
        ...
        page:(
          await Promise.all(
            results.page.map(async(message)=> {
              ...
              const reactions=await populationReaction(ctx,message._id);
              const thread=await populationThread(ctx,message._id);
              ...
              const reactionWIthoutMemberId=dedupedReactions.map(
                ({memberId,...rest})=>rest
              );
